...
function MediaCard({ movie }: MediaCardProps) {
  const isTv = movie.media_type === 'tv' || movie.type === 'tv' || (!movie.release_date && Boolean(movie.first_air_date))
...
export default function HomePage() {
  ...
  // Fetch all home data in a single coordinated lifecycle
  useEffect(() => {
    let isMounted = true
    ...
    const fetchAllData = async () => {
      setLoading(true)
      try {
        // Fetch public TMDB categories concurrently
        const [popRes, tvRes, nowRes, topRes, topTvRes] = await Promise.all([
  ...
  // Complex Search live dropdown previews with AbortController
  useEffect(() => {
    if (!searchQuery.trim()) {
      setSuggestions([])
  ...
  // Filter user top 10 list depending on active tab
  const filteredUserTopTen = userTopTen.filter((item) => {
    if (activeTab === 'movies') return item.type !== 'tv'
    if (activeTab === 'tv') return item.type === 'tv'
    return true
  })

  // Determine currently focused item in Top 10 hero banner
  const activeTopTenItem = filteredUserTopTen[hoveredTopTenIndex] || filteredUserTopTen[0]
      ...
      <header className="sticky top-0 z-50 w-full bg-black/90 backdrop-blur-xl border-transparent px-6 py-5 flex flex-col items-center gap-5 shadow-2xl">
        ...
        <div className="flex items-center bg-zinc-900/90 p-1 rounded-2xl border border-zinc-800/80 shadow-lg">
          <button
            ...
            className={`px-6 py-2 rounded-xl text-xs sm:text-sm font-bold transition duration-200 ${
              activeTab === 'all'
          ...
          <div className="max-w-7xl mx-auto w-full px-6 relative z-10 flex flex-col justify-between gap-8 h-full">
            <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 min-h-[160px]">
              <div className="flex flex-col">
                ...
                <h2 className="text-4xl sm:text-6xl font-black text-white tracking-tight drop-shadow-xl">
                  YOUR TOP 10
                </h2>
              </div>
                ...
                <div className="flex flex-col items-start sm:items-end text-left sm:text-right gap-2 max-w-xl">
                  <div className="flex items-center gap-2">
                    <span className="bg-amber-400 text-black px-2.5 py-0.5 rounded-md text-xs font-black uppercase tracking-wider shadow-md">
                      #{ (hoveredTopTenIndex < filteredUserTopTen.length ? hoveredTopTenIndex : 0) + 1 } IN RANKING
                    </span>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold text-white uppercase tracking-wider ${
                      activeTopTenItem.type === 'tv' ? 'bg-sky-600/90' : 'bg-purple-600/90'
                    }`}>
                      {activeTopTenItem.type === 'tv' ? 'TV SHOW' : 'MOVIE'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-xs sm:text-sm font-semibold text-zinc-300 mt-1">
                    ...
                    <Link
                      href={`/${activeTopTenItem.type === 'tv' ? 'tv' : 'movie'}/${activeTopTenItem.tmdb_id}`}
                      className="bg-white/10 hover:bg-white/20 text-white px-3.5 py-1 rounded-xl border border-white/20 backdrop-blur-md transition duration-200 font-bold flex items-center gap-1.5"
                    >
                      Details →
                    </Link>
                  </div>
            ...
            <div className="flex gap-4 overflow-x-auto pt-4 pb-4 scrollbar-thin scrollbar-thumb-zinc-800 items-start">
              {filteredUserTopTen.map((media, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Link
                      ...
                      className="w-36 sm:w-44 flex flex-col"
                    >
